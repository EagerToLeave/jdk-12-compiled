...
#define inc_counter_np(counter) \
  ...
  address generate_call_stub(address& return_address) {
    ...
    // 这里先得到当前入口的内存地址, 因为此函数后续流程会继续往代码空间中写入call_stub()的指令, 而外面只需要获取这部分指令的
    // 起始地址, 此处的start即为例程所在内存的起始地址, 此处先获取首地址, 并未对其进行操作, 最后直接将其返回
    address start = __ pc();
    ...
    // stub code
    __ enter();                                  // 以x86为例, macroAssembler_x86.cpp中, enter()函数实现为: push(rbp); mov(rbp, rsp); 两行代码均调用emit(),也即向代码空间中写入机器码
    __ movptr(rcx, parameter_size);              // parameter counter, 此处看似汇编的指令的C语言函数并不实在运行时真正被调用的函数, 主要作用在于往代码空间中写入具有相同作用的机器码
