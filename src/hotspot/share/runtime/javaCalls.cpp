  ...
  { JavaCallWrapper link(method, receiver, result, CHECK);
    { HandleMark hm(thread);  // HandleMark used by HandleMarkCleaner
      ...
      StubRoutines::call_stub()(
        ...
        entry_point,                 // JVM调用Java方法的例程入口，JVM内部的每一段例程都是在JVM启动过程中预生成好的一段机器指令; 需要调用Java方法，都需经历此例程（需先执行这段机器指令，然后才能跳转到Java方法对应的机器指令去执行）
        args->parameters(),          // Java方法的入参集合
        args->size_of_parameters(),  // Java方法入参数量
        CHECK                        // 当前线程对象
      );
